...
### 测试页面
- **index.html**: 主应用（模块化版本）
- **standalone.html**: 单文件版本（不使用模块化）
- **simple-test.html**: 简单的模块加载测试
- **test.html**: 完整的自动化测试套件
- **debug.html**: 模块加载验证工具

**使用方法**: 
1. 首先测试 `standalone.html`（确保基本功能正常）
2. 然后测试 `simple-test.html`（检查模块加载）
3. 最后测试 `index.html`（完整应用）
...
#### 题目难度调整
- **加减法**
  - 加法：数字范围扩大至 10-90，100% 为进位加法
  - 减法：70% 为退位减法，数字范围 50-99
